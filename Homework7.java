...
class InkjetPrinter extends Printer {
    ...
    public boolean print() {
        if (availableCount == 0) {
            System.out.println(model + ": " + (countOfprinted + 1) + "매째 인쇄 실패 - 잉크 부족.");
            return false;
        } else {
            countOfprinted++;
            availableCount--;
...
class LaserPrinter extends Printer {
    ...
    public boolean print() {
        if (availableCount == 0) {
            System.out.println(model + ": " + (countOfprinted + 1) + "매째 인쇄 실패 - 토너 부족.");
            return false;
        } else {
            countOfprinted++;
            availableCount--;
